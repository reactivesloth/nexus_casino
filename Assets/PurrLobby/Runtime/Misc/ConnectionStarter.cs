...
namespace PurrLobby
{
    public class ConnectionStarter : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            if (_networkManager.transport is UDPTransport)
            {
        ...
        private IEnumerator UDPConnect()
        {
            yield return new WaitUntil(() => PlayFlowLobbyManagerV2.Instance.CurrentLobby.GetGameServerStatus() == "running");
            
            var transport = _networkManager.transport as UDPTransport;
            if (transport != null)
            {
                var connection = PlayFlowLobbyManagerV2.Instance.GetGameServerConnectionInfo();
                if (connection != null)
                {
                    transport.address = connection.Value.Ip;
                    transport.serverPort = (ushort)connection.Value.Port;
