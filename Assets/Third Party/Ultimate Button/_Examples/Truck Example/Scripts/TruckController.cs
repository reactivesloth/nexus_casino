...
public class TruckController : MonoBehaviour
{
	...
	void Update ()
	{
		...
		UltimateButton.ReturnComponent( "ResetTruck" ).UpdateCooldown( resetTruckTimer, resetTruckTimerMax );
	}
	...
	void FixedUpdate ()
	{
		...
		Vector2 myVelocity = myRigidbody.linearVelocity;
#else
		...
		else
		{
			...
			else
			{
				...
				if( isGrounded == true && motorSpeed > -100 && myVelocity.x > 2 )
		...
		if( !isGrounded && myVelocity.magnitude <= 0.5f && !isResetting )
		{
	...
	IEnumerator ResetTruckCoroutine ()
	{
		...
		// Set the rigidbody to Kinematic to not allow physics movements to it.
...
#if UNITY_6000_0_OR_NEWER
		backRigidbody.linearVelocity = Vector2.zero;
		frontRigidbody.linearVelocity = Vector2.zero;
#else
