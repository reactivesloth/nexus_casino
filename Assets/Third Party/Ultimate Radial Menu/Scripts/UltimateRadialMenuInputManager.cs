...
public class UltimateRadialMenuInputManager : MonoBehaviour
{
	...
	public class UltimateRadialMenuInformation
	{
		...
		public Vector2 previousTouchPos = Vector2.zero;
		public float previousTouchDist = 0.0f;
	...
	public List<UltimateRadialMenuInformation> UltimateRadialMenuInformations
	{
		get;
		private set;
	}
	...
	bool customInputEnable = false, customInputDisable = false;
	...
	[Tooltip( "Determines whether or not the Ultimate Radial Menu will interact with the current button when the menu is disabled." )]
	...
	[Tooltip( "Should the touch input position be stored and used even if the touch is released?" )]
	public bool retainTouchPosition = false;
	...
	public UnityEngine.InputSystem.InputAction customControllerJoystick;
	public UnityEngine.InputSystem.InputAction customControllerInteract;
	public UnityEngine.InputSystem.InputAction customControllerEnable;
	...
	protected virtual void Awake ()
	{
		...
		if( !GetComponent<EventSystem>() && !GetComponent<UltimateRadialMenu>() )
		{
			...
			Debug.LogError( FormatDebug( "This component is not attached to the EventSystem in your scene or an Ultimate Radial Menu component", "Please make sure that you have only one Ultimate Radial Menu Input Manager in your scene and that it is located on the EventSystem. If you want unique controller input then place the Ultimate Radial Menu Input Manager on the Ultimate Radial Menu GameObject that you want to have unique controller input on", gameObject.name ) );
		...
		UltimateRadialMenuInformations = new List<UltimateRadialMenuInformation>();
	...
	public void AddRadialMenuToList ( UltimateRadialMenu radialMenu, ref UltimateRadialMenuInputManager inputManager )
	{
		...
		UltimateRadialMenuInformation newInfo = new UltimateRadialMenuInformation() { radialMenu = radialMenu, inputManager = this };
		...
		if( touchInput )
			radialMenu.OnMenuDisabled += newInfo.ResetMenuPosition;
	...
	protected void CheckEnableMenu ( bool buttonPressed, bool buttonReleased, int radialMenuIndex )
	{
		...
		if( enableMenuSetting == EnableMenuSetting.Hold )
		{
			...
			if( buttonPressed && !UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.IsEnabled )
				UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.Enable();

			// If the button has been released and the radial menu is active, then disable it.
			if( buttonReleased && UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.IsEnabled )
				UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.Disable();
		}
	...
	{
		return "<b>Ultimate Radial Menu Input Manager</b>\n" +
			"<color=red><b>×</b></color> <i><b>Error:</b></i> " + error + ".\n" +
			"<color=green><b>√</b></color> <i><b>Solution:</b></i> " + solution + ".\n" +
			"<color=#00FFFF><b>∙</b></color> <i><b>Object:</b></i> " + objectName + "\n";
	}
	...
	private void Update ()
	{
		...
		for( int i = 0; i < UltimateRadialMenuInformations.Count; i++ )
		{
			...
			if( customInputCalculated )
			{
				...
				if( customInputUp )
					inputUp = true;
				if( customInputEnable )
					enableMenu = true;
				if( customInputDisable )
					disableMenu = true;
			}

			// If the user wants to send the interact action when the menu is going to be disabled, and the menu is currently enabled visually...
			if( onMenuRelease && UltimateRadialMenuInformations[ i ].radialMenu.IsEnabled )
			{
				// If the user wants to hold the menu to stay active, and the button is released, OR if the user wants to toggle the menu state and the button has been pressed, then set the input down/up to true.
				if( enableMenuSetting == EnableMenuSetting.Hold && disableMenu || enableMenuSetting == EnableMenuSetting.Toggle && enableMenu )
		...
		if( customInputCalculated )
		{
			...
			customInputEnable = customInputDisable = false;
		}
	}
	...
	public virtual void MouseAndKeyboardInput ( ref bool enableMenu, ref bool disableMenu, ref Vector2 input, ref float distance, ref bool inputDown, ref bool inputUp, int radialMenuIndex )
	{
		...
		if( !mouse.enabled && Touchscreen.current != null && Touchscreen.current.enabled )
			Debug.LogError( FormatDebug( "Mouse device is disabled by Unity", "Please disable the Simulated Touchscreen option. To do this, go to Window > Analysis > Input Debugger. In this window, disable the Simulated Touchscreen by going to Options > Simulate Touch Input From Mouse Or Pen", "NA" ) );
	...
	/// </summary>
	private void CustomControllerInputDown ( UnityEngine.InputSystem.InputAction.CallbackContext obj )
	{
	...
	/// [INTERNAL] For Unity's new Input System.
	/// </summary>
	private void CustomControllerInputUp ( UnityEngine.InputSystem.InputAction.CallbackContext obj )
	{
		customControllerCalculated = true;
	...
	/// [INTERNAL] For Unity's new Input System.
	/// </summary>
	private void CustomControllerEnable ( UnityEngine.InputSystem.InputAction.CallbackContext obj )
	{
		customControllerCalculated = true;
	...
	/// [INTERNAL] For Unity's new Input System.
	/// </summary>
	private void CustomControllerDisable ( UnityEngine.InputSystem.InputAction.CallbackContext obj )
	{
		customControllerCalculated = true;
	...
	public virtual void TouchInput ( ref bool enableMenu, ref bool disableMenu, ref Vector2 input, ref float distance, ref bool inputDown, ref bool inputUp, int radialMenuIndex )
	{
			...
			for( int i = 0; i < touchCount; i++ )
			{
...
#if ENABLE_INPUT_SYSTEM
				touchPosition = UnityEngine.InputSystem.EnhancedTouch.Touch.activeTouches[ i ].screenPosition;
				touchBegan = UnityEngine.InputSystem.EnhancedTouch.Touch.activeTouches[ i ].phase == UnityEngine.InputSystem.TouchPhase.Began;
				touchEnded = UnityEngine.InputSystem.EnhancedTouch.Touch.activeTouches[ i ].phase == UnityEngine.InputSystem.TouchPhase.Ended;
				fingerId = UnityEngine.InputSystem.EnhancedTouch.Touch.activeTouches[ i ].touchId;
#else
				...
				if( touchBegan )
				{
					...
					if( UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.IsEnabled )
					{
						...
						// If the distance is NOT within the min input range of the radial menu, then reset the stored input position.
						if( retainTouchPosition && dist < UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.CalculatedMinRange )
						{
							// Store the previous input for possible use next frame.
							UltimateRadialMenuInformations[ radialMenuIndex ].previousTouchPos = Vector2.zero;
							UltimateRadialMenuInformations[ radialMenuIndex ].previousTouchDist = 0.0f;
						}
					}
				}

				// If the finger id is still not stored, then just continue to the next touch.
				if( !retainTouchPosition && UltimateRadialMenuInformations[ radialMenuIndex ].interactFingerID == -1 )
					continue;

				// If the radial menu is not active and not in the middle of a transition...
				if( enableMenuSetting != EnableMenuSetting.Manual && !UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.IsWorldSpaceRadialMenu && !UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.IsEnabled )
				{
				...
				if( touchEnded )
				{
					...
					// If the enable menu setting is manual, or the activation range is zero, then just set inputUp to true.
					if( enableMenuSetting == EnableMenuSetting.Manual || activationRadius <= 0.0f )
						inputUp = true;
					// Else set input down and up to true so that the interact function will be called on the button when releasing.
					else
						inputDown = inputUp = true;
		...
		// If the user wants to retain the input of the touch input...
		if( retainTouchPosition )
		{
			// If the interact finger ID is reset, then copy the previously known input value.
			if( UltimateRadialMenuInformations[ radialMenuIndex ].interactFingerID < 0 )
			{
				input = UltimateRadialMenuInformations[ radialMenuIndex ].previousTouchPos;
				distance = UltimateRadialMenuInformations[ radialMenuIndex ].previousTouchDist;
			}

			// Store the previous input for possible use next frame.
			UltimateRadialMenuInformations[ radialMenuIndex ].previousTouchPos = input;
			UltimateRadialMenuInformations[ radialMenuIndex ].previousTouchDist = distance;
		}
	}
	...
	public virtual void CenterScreenInput ( ref bool enableMenu, ref bool disableMenu, ref Vector2 input, ref float distance, ref bool inputDown, ref bool inputUp, int radialMenuIndex )
	{
		...
		if( virtualReality )
		{
			...
			{
				Debug.LogError( FormatDebug( "The left or right eye cameras are unassigned", "Please ensure that they are assigned in the inspector. If you need to update them at runtime, please use the SetCamerasVR() function", gameObject.name ) );
				return;
			}
		...
		if( centerScreenDistance > 0 && centerScreenDistance <= UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.CalculatedMaxRange )
		{
			...
			if( enableWhenInRange && !UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.IsEnabled )
				UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.Enable();
		...
		else if( ( centerScreenDistance > UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.CalculatedMaxRange || centerScreenDistance <= 0.0f ) && UltimateRadialMenuInformations[ radialMenuIndex ].radialMenu.IsEnabled )
		{
	...
	/// <summary>
	/// Triggers interact with the radial menu.
	/// </summary>
	public void TriggerInputInteract ()
	{
		customInputCalculated = true;
		customInputDown = customInputUp = true;
	}

	/// <summary>
	/// Triggers the radial menus to enable.
	/// </summary>
	...
	/// <summary>
	/// Triggers the radial menus to disable.
	/// </summary>
	public void TriggerDisable ()
	{
		customInputCalculated = true;
		customInputDisable = true;
	}
}