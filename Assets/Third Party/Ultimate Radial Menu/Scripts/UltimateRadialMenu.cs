...
public class UltimateRadialMenu : MonoBehaviour
{
	...
	public class UltimateRadialButton
	{
		...
		public void ClearButtonInformation ()
		{
			...
			// Exit the button to reset everything to default values.
			OnExit();
			...
			// If the text component is assigned then reset the text value.
			if( text != null )
				text.text = "";
	...
	/// <summary>
	/// Called when a radial button has been entered.
	/// </summary>
	public event Action<int> OnButtonEnter;
	/// <summary>
	/// Called when a radial button has been exited.
	/// </summary>
	...
	/// <summary>
	/// Called when the input has been pressed on a radial button.
	/// </summary>
	public event Action<int> OnButtonInputDown;
	/// <summary>
	/// Called when the input has been released on a radial button.
	/// </summary>
	public event Action<int> OnButtonInputUp;
	/// <summary>
	/// Called when a radial button has been interacted with.
	/// </summary>
	public event Action<int> OnButtonInteract;
	/// <summary>
	/// Called when a radial button has been selected.
	/// </summary>
	public event Action<int> OnButtonSelected;
	/// <summary>
	/// Called when the radial menu gains focus.
	/// </summary>
	public event Action OnMenuFocused;
	/// <summary>
	/// Called when the radial menu has lost focus.
	/// </summary>
	public event Action OnMenuLostFocus;
	/// <summary>
	/// Called when the radial menu has been enabled.
	/// </summary>
	public event Action OnMenuEnabled;
	/// <summary>
	/// Called when the radial menu has been disabled.
	/// </summary>
	public event Action OnMenuDisabled;
	/// <summary>
	/// Called when the radial menu's positioning has been updated.
	/// </summary>
	public event Action OnUpdatePositioning;
	/// <summary>
	/// Called when a radial button is added or subtracted from the Radial Menu during this frame. This is useful for swapping sprites and positioning for a new count.
	/// </summary>
	public event Action<int> OnButtonCountModified;
	/// <summary>
	/// Relays the information that is provided from the Ultimate Radial Menu Input Manager script according to the user's settings.
	/// </summary>
	...
	void Start ()
	{
		...
		else
		{
			...
			if( eventSystem != null && !eventSystem.gameObject.GetComponent<UltimateRadialMenuInputManager>() )
			{
				Debug.LogWarning( FormatDebug( "There was no Ultimate Radial Menu Input Manager on the EventSystem in your scene. Adding a default Ultimate Radial Menu Input Manager to avoid errors", "Please exit play mode and ensure that you have an Ultimate Radial Menu Input Manager on your EventSystem so that you can customize the settings", eventSystem.gameObject.name + "(No Ultimate Radial Menu Input Manager component)" ) );

				eventSystem.gameObject.AddComponent<UltimateRadialMenuInputManager>().AddRadialMenuToList( this, ref inputManager );
			}
	...
	/// <summary>
	/// [INTERNAL] Processes the input and calculates the information for use within the Ultimate Radial Menu.
	/// </summary>
	...
	void ProcessPointer ()
	{
		...
		else if( pointerSnapOption == PointerSnapOption.Smooth )
	...
	void RadialMenuButtonCountModified ()
	{
		...
		if( radialMenuStyle != null )
		{
			...
			for( int i = 0; i < radialMenuStyle.RadialMenuStyles.Count; i++ )
			{
				...
				// If the calculated angle is less than or equal to the current needed angle... (NOTE: Casting as a "redundant" float because Unity 2020.1+ has a bug that prevents this conditional from catching 7 and 11 button counts)
				if( ( float )styleAngle <= GetAnglePerButton )
				{
	...
	public void UpdatePositioning ()
	{
		...
		// If the user wants an offset, then adjust the base transforms position by the set value.
		if( overallAngle < 360.0f && centerOffset != -Vector2.one )
			BaseTransform.localPosition -= ( Vector3 )( ( BaseTransform.sizeDelta / 2 ) - BaseTransform.sizeDelta * centerOffset );
		...
		//if( ParentCanvas.renderMode != RenderMode.ScreenSpaceOverlay )// EDIT: 
		...
		for( int i = 0; i < UltimateRadialButtonList.Count; i++ )
		{
			...
			if( UltimateRadialButtonList[ i ].text != null )
			{
				...
				else
				{
					...
					//if( IsWorldSpaceRadialMenu )// EDIT:
