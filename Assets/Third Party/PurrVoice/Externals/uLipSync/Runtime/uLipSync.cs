...
{
    [HideInInspector, SerializeField] private Profile profile;
    [HideInInspector] public LipSyncUpdateEvent onLipSyncUpdate = new LipSyncUpdateEvent();
    [Range(0f, 1f), HideInInspector] public float outputSoundGain = 1f;
    ...
    readonly List<int> _requestedCalibrationVowels = new List<int>();
    readonly Dictionary<string, float> _ratios = new Dictionary<string, float>();
    ...
    public void OnDataReceived(ArraySegment<float> input, int channels)
    {
    ...
    protected virtual void OnAudioFilterRead(float[] input, int channels)
    {
