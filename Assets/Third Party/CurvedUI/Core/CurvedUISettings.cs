...
#define CURVEDUI_PRESENT //If you want to see if CurvedUI is imported, just use "#if CURVEDUI_PRESENT [your code] #endif"
using System;
using UnityEngine;
...
{
    /// <summary>
    /// This class stores settings for the entire canvas. It also stores useful methods for converting coordinates
    /// to and from 2d canvas to curved canvas, or world space. CurvedUIVertexEffect components, which are added to
    /// every canvas GameObject, ask this class for per-canvas settings when applying the curve effect.
    /// </summary>
...
#region PUBLIC
        ...
        {
            foreach (var eff in this.GetComponentsInChildren<CurvedUIVertexEffect>())
            {
        ...
        public ControlMethod ControlMethod {
            get => CurvedUIInputModule.ActiveControlMethod;
            set => CurvedUIInputModule.ActiveControlMethod = value;
        }
