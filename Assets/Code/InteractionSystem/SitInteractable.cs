...
namespace Code.InteractionSystem
{
  public class SitInteractable : Interactable
  {
    ...
    [SerializeField] private EntryData[] entries = System.Array.Empty<EntryData>();
    ...
    private const string K_IS_SITTING = "sit.isSitting";
    private const string K_ENTRY_IDX  = "sit.entryIndex";
    ...
    {
      base.OnStartServer();
      RegisterBoolSlot(K_IS_SITTING, false);
      RegisterIntSlot(K_ENTRY_IDX, 0);
      SetBool(K_IS_SITTING, false);
      SetInt(K_ENTRY_IDX, 0);
    }
    ...
    protected internal override void OnInteract(NetworkConnection conn, bool force)
    {
      ...
      if (!GetBool(K_IS_SITTING))
        RequestSetBool(K_IS_SITTING, true, conn);

      TargetToggleSitLocal(isSitDown: true, isForce: force);
    }
    ...
    private void HandleSyncedChanged(string key, object prev, object next, bool asServer)
    {
      if (key == K_IS_SITTING || key == K_ENTRY_IDX)
      {
        ...
        bool isSit = GetBool(K_IS_SITTING);
        TargetToggleSitLocal(isSitDown: isSit);
      }
    }
    ...
    private void HandleForceApply()
    {
      ...
      ApplySitStateImmediate(
        ...
        entryIndex: GetInt(K_ENTRY_IDX)
      );
      _didInitialApply = true;
    }

    private void TargetToggleSitLocal(bool isSitDown, bool isForce = false)
    {
      ...
      var entry = GetEntry(GetInt(K_ENTRY_IDX)) ?? FindClosestEntryPoint(tf.position) ?? GetEntry(0);
                                    ...
                                    Transform tf, EntryData entry)
    {
      ...
      if (entry == null)
      {
        Debug.LogWarning("SitInteractable: No entry point found", this);
        IsBusy = false;
        yield break;
      }
      ...
      Vector3 targetPos = (sitPoint ? sitPoint.position : tf.position) + Vector3.up * footOffset + Vector3.up * sitAdjustHeight;
