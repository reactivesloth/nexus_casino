...
public class UltimateRadialMenuInputManagerEditor : Editor
{
	...
	private void OnEnable ()
	{
		...
		UltimateRadialMenuInputManager[] allInputManagers = FindObjectsByType<UltimateRadialMenuInputManager>( FindObjectsSortMode.None );
#else
	...
	public override void OnInspectorGUI ()
	{
...
#endif
				if( !eventSystem.gameObject.GetComponent<UltimateRadialMenuInputManager>() )
					eventSystem.gameObject.AddComponent<UltimateRadialMenuInputManager>();

				multipleInputManagerError = false;
			}
		...
		else
		{
			...
			EditorGUI.BeginDisabledGroup( targ.enableMenuSetting == UltimateRadialMenuInputManager.EnableMenuSetting.Manual );
			EditorGUILayout.PropertyField( serializedObject.FindProperty( "onMenuRelease" ), new GUIContent( "Interact On Menu Disable" ) );
			EditorGUI.EndDisabledGroup();
			...
			else
			{
				...
				if( DisplayCollapsibleBoxSection( "Touch Input", "URMIM_TouchInput", serializedObject.FindProperty( "touchInput" ) ) )
				{
					...
					EditorGUI.BeginChangeCheck();
					EditorGUILayout.PropertyField( serializedObject.FindProperty( "retainTouchPosition" ) );
					if( EditorGUI.EndChangeCheck() )
						serializedObject.ApplyModifiedProperties();
				}
				EditorGUILayout.EndVertical();
				...
				if( DisplayCollapsibleBoxSection( "Center Screen Input", "URMIM_CenterScreenInput", serializedObject.FindProperty( "centerScreenInput" ) ) )
				{
					...
					if( serializedObject.FindProperty( "enableWhenInRange" ).boolValue )
					{
						...
						{
							SerializedObject radialMenuObject = new SerializedObject( allRadialMenus[ i ] );
							if( allRadialMenus[ i ].IsWorldSpaceRadialMenu && radialMenuObject.FindProperty( "menuToggleOverTime" ).boolValue && radialMenuObject.FindProperty( "menuToggleScale" ).boolValue )
							{
	...
	private void OnSceneGUI ()
	{
		...
		for( int i = 0; i < allRadialMenus.Length; i++ )
		{
			if( targ.centerScreenInput && EditorPrefs.GetBool( "URMIM_CenterScreenInput" ) && allRadialMenus[ i ].IsWorldSpaceRadialMenu )
			{
				...
				SerializedObject radialMenuObject = new SerializedObject( allRadialMenus[ i ] );
				if( serializedObject.FindProperty( "enableWhenInRange" ).boolValue && allRadialMenus[ i ].IsWorldSpaceRadialMenu && radialMenuObject.FindProperty( "menuToggleOverTime" ).boolValue && radialMenuObject.FindProperty( "menuToggleScale" ).boolValue )
				{
