...
namespace CC
{
    ...
    public class ModifyBone_Manager : MonoBehaviour
    {
        ...
        // Кэш модификаторов по типу, чтобы не аллоцировать LINQ-ом каждый раз.
        ...
        private void Awake()
        {
            // Не полагаемся на то, что нас вызовут раньше других.
            EnsureInitialized();
        }

        private void OnEnable()
        {
            // На случай, если иерархию меняли во время отключения.
            _byType = null;
        }

        /// <summary>
        /// Гарантирует, что кэш и ссылки готовы.
        /// Вызывается из каждого публичного пути использования.
        /// </summary>
        ...
        /// <summary>
        /// Перестроить кэш модификаторов (можно вызвать вручную, если динамически меняете детей).
        /// </summary>
        public void RebuildCache()
        {
            // includeInactive = true — чтобы подхватить модификаторы на выключенных объектах, если они появятся позже.
        ...
        /// <summary>
        /// Быстрый доступ к списку модификаторов нужного типа. Никогда не возвращает null.
        /// </summary>
            ...
            switch (modifyType)
            {
                ...
                case "BodyCustomization_Weight":
                    if (customizer != null)
                    {
                        ...
                        // Оставил ваши закомментированные строки как есть.
