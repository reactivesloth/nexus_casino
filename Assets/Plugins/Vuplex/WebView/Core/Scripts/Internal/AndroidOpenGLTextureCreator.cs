...
namespace Vuplex.WebView.Internal {
    ...
    public class AndroidOpenGLTextureCreator : MonoBehaviour {

        public static AndroidOpenGLTextureCreator GetInstance(IntPtr nativeCreateTextureFunction) {
            if (!_instance) {
                _instance = (AndroidOpenGLTextureCreator) new GameObject("AndroidOpenGLTextureCreator").AddComponent<AndroidOpenGLTextureCreator>();
                _instance._nativeCreateTextureFunction = nativeCreateTextureFunction;
        ...
        static AndroidOpenGLTextureCreator _instance;
        List<int> _invocationIDsToTrigger = new List<int>();
        IntPtr _nativeCreateTextureFunction;
            ...
            while (true) {
                ...
                if (_invocationIDsToTrigger.Count > 0) {
                    foreach (var invocationID in _invocationIDsToTrigger) {
                        GL.IssuePluginEvent(_nativeCreateTextureFunction, invocationID);
                    }
        ...
        void HandleTextureCreated(string parameterString) {
            ...
            var parameters = parameterString.Split(new []{';'}, 2);
